...
- When the provider is `llamacpp` (or `local`), Qwery creates a provider that speaks OpenAI-compatible HTTP to your local `llama-server` (`packages/agent-factory-sdk/src/services/model-resolver.ts:50-60`, `packages/agent-factory-sdk/src/services/models/llamacpp-model.provider.ts:12-49`).
...
LLAMACPP_MODEL=<model-id-from-/v1/models>
```
...
- `LLAMACPP_BASE_URL`: base URL of the OpenAI-compatible server, defaults to `http://127.0.0.1:8080/v1`
- `LLAMACPP_MODEL`: model id returned by `GET /v1/models` on your `llama-server`
